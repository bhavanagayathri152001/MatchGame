...
class App extends Component {
  ...
  componentDidMount = () => {
    this.timerId = setInterval(this.timerSeconds, 1000)
  }
  ...
  onClickTabItem = tabId => this.setState({activeTabId: tabId})
  ...
  onClickThumbnail = thumbnailUrl => {
    ...
    if (imgUrl === randomImgUrl) {
      const randomNumber = Math.floor(Math.random() * imagesList.length)
      const newImgUrl = imagesList[randomNumber].imageUrl
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImgUrl: newImgUrl,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({isTrue: false})
        ...
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <li key={eachTab.tabId}>
              <button
                type="button"
                className={`tab-display-text-btn ${
                  activeTabId === eachTab.tabId ? 'active-class-name' : ''
                }`}
          ...
          {filteredList.map(eachImage => (
            <li key={eachImage.id}>
              <button
                ...
                onClick={() => this.onClickThumbnail(eachImage.thumbnailUrl)}
              >
                <img
  ...
  render() {
    const {seconds, isTrue, score} = this.state
    return (
          ...
          <div className="score-timer-container">
            ...
            <div className="timer-container">
              <img
                ...
                className="timer"
              />
              <p className="seconds-text">{`${seconds} sec`}</p>
            </div>
          </div>
        </ul>
        {isTrue ? this.matchGameView() : this.scoreCardView()}
      </div>
    )
  }
}
